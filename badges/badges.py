...
class Badges(commands.Cog):
    ...
    def make_template(self, user, badge, template):
       ...
       #pastes the barcode, unused in this instance
    ...
    def make_animated_gif(self, template, avatar):
        ...
        for frame in gif_list:
            temp2 = template.copy()
           #watermark is unused, meant for another cog
            #watermark = frame.copy()
            #watermark = frame.convert("RGBA")
            #watermark = watermark.resize((100, 100))
            #watermark.putalpha(128)
            frame = frame.convert("RGBA")
            id_image = frame.resize((1200, 1200))
            frame = temp2.resize((1200, 1200))
            position = ((frame.width - id_image.width), (frame.height - id_image.height))
            #temp2.paste(watermark, (845, 45, 945, 145), watermark)
            #temp2.paste(id_image, (60, 95, 225, 260))
            temp2.paste(id_image, position, id_image)
            temp2.paste(frame, position, frame)
    ...
    def make_badge(self, template, avatar):
       ...
       id_image = avatar.resize((1200, 1200))
       avatar = template.resize((1200, 1200))
       position = ((avatar.width - id_image.width), (avatar.height - id_image.height))
       template.paste(id_image, position, id_image)
       template.paste(avatar, position, avatar)
       temp = BytesIO()
    ...
    async def badges(self, ctx, *, badge):
        ...
        if not badge_obj:
            await ctx.send(_("`{}` is not an available badge color.").format(badge))
            return
        async with ctx.channel.typing():
            badge_img = await self.create_badge(user, badge_obj, True)
            if badge_img is None:
                await ctx.send(_("Something went wrong while collecting your profile picture."))
                return
            image = discord.File(badge_img)
            await ctx.send(file=image, content=":art: **| Enjoy your new profile picture, {}!**".format(user.mention))
   ...
   #causes errors, is useless, and too lazy to fix
    #@commands.command()
